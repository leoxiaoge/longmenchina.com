...
if ($action=="confirm"){
	...
	for($i=0;$i<$del_num;$i++){
		$sql="select img1 FROM `{$tablepre}news` where id=".$checkid[$i]."";
...
$sql="select * FROM `{$tablepre}news` where 1=1 {$sqlkey} order by disorder desc,id asc";
//echo $sql;
...
<body>
    ...
    <div class="rightinfo">
    <form name="formlist" method="post" action="?action=del&pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>&ttty=<?=$ttty?>">

    <div class="tools">
    
    	<ul class="toolbar">
			<li><a href="gg.php?pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>&ttty=<?=$ttty?>&<?=$url?>"><img src="images/sx.jpg" /></a></li>
			<li>&nbsp;<input type="checkbox" name="all" onClick="CheckAll(this);"> 全选</li>
			<li><a href="gg_add.php?pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>&ttty=<?=$ttty?>"><img src="images/add.jpg" /></a></li>
   ...
   <?
	...
	?>
        ...
        <td><?=$bd['title']?></td>
        <td><?=$zt?></td>
        <td><a href="<?=$path?>/<?=$Img?>" target="_blank">预览</a></td>
