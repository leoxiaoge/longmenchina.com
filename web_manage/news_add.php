...
$mycounts=get_count(" and pid={$pid}","news")+1;
		...
		<div id="tab1" class="tabson">
			...
			<ul class="forminfo">
				...
				<form action="" method="post" name="formlist" enctype="multipart/form-data"  onSubmit="return news_check(this);">
					...
					for ($i=1;$i<=$trcount;$i++){?>
					<li><label>封面图片<b>*</b></label><input name="img<?=$i?>" type="file" id="img<?=$i?>"> <font color="#FF0000">图片大小:<?=$System_Picsize?>K内,图片尺寸：<?=$arrValue[$i-1]?>px</font></li>
					<? } ?>
					<? } ?>
		
					<li><label>创建时间<b>*</b></label><input name="info[sendtime]" type="text" class="dfinput" value="<?=$PHP_DATE?>" style="width:150px;"/></li>
		
					<li><label>信息排序<b>*</b></label><input name="info[disorder]" type="text" class="dfinput" value="0" style="width:100px;"/> 注：数字越大越排在前 参考数值1- <?=$mycounts?> </li>			
