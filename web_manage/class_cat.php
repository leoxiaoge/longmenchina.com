...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar2">
            ...
            <li><a href="class_cat_add.php">添加栏目</a></li>
        </ul>
...
?>
            ...
            <tr>
            	<td width="5%"><?=$pid?></td>
                <td width="10%" align="left" class="tl">【<?=$bd1['catname']?>】 <?=$webarr["iscats"][$bd1['iscats']]?></td>
                <td width="10%"><?=$webarr["showtype"][$bd1['showtype']]?></td>
                <td width="10%"><?=get_count(" and pid={$pid} ")?></td>
                <td width="10%"><a href="../list.php?pid=<?=$pid?>" target="_blank">访问</a></td>
                <td width="10%"><?
		  	if($bd1['isstate']=="1"){
				 echo("<a href='?action=confirm&id=".$bd1['id']."'><font color='#FF00FF'>已审核</font></a>");
			}else{
				 echo("<a href='?action=confirm&id=".$bd1['id']."'><font color='#FF0000'>未审核</font></a>");
			 }
			?></td>
                <td width="35%"><a href="class_cat_add.php?pid=<?=$pid?>" style="color:red">添加子栏目</a>&nbsp;|&nbsp;<a href="class_cat_edit.php?id=<?=$bd1['id']?>">修改</a>&nbsp;|&nbsp;<a href="?action=del&pid=<?=$pid?>&id=<?=$bd1['id']?>" onClick="return ConfirmDelBig();">删除</a></td>
            </tr>
	<?
	...
	?>
			...
			?></td>
				<td width="35%"><a href="class_cat_add.php?pid=<?=$pid?>&ty=<?=$ty?>" style="color:blue">添加子栏目</a>&nbsp;|&nbsp;<a href="class_cat_edit.php?id=<?=$bd2['id']?>">修改</a>&nbsp;|&nbsp;<a href="?action=del&pid=<?=$pid?>&ty=<?=$ty?>&id=<?=$bd2['id']?>" onClick="return ConfirmDelSmall();">删除</a></td>
            </tr>
	<?
	...
	?>
		    <tr>
            	<td width="5%"><?=$bd3['id']?></td>
                <td width="10%" align="left" class="tl">&nbsp;&nbsp;└└<?=$bd3['catname']?></td>
                <td width="10%"><?=$showtype?></td>
                <td width="10%"><?=get_count(" and pid={$pid} and ty={$ty} and tty={$tty} ")?></td>
                <td width="10%"><a href="../list.php?pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>" target="_blank">访问</a></td>
                <td width="10%"><?
		  	if($bd3['isstate']=="1"){
			...
			?></td>
			    <td width="35%"><a href="class_cat_add.php?pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>" style="color:green">添加子栏目</a>&nbsp;|&nbsp;<a href="class_cat_edit.php?id=<?=$bd3['id']?>">修改</a>&nbsp;|&nbsp;<a href="?action=del&pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>&id=<?=$bd3['id']?>" onClick="return ConfirmDelSmall1();">删除</a></td>
            </tr>
	<?
	...
	?>
		    <tr>
                ...
                <td width="10%"><?=$showtype?></td>
                <td width="10%"><?=get_count(" and pid={$pid} and ty={$ty} and tty={$tty} and ttty={$ttty} ")?></td>
                <td width="10%"><a href="../list.php?pid=<?=$pid?>&ty=<?=$ty?>&tty=<?=$tty?>" target="_blank">访问</a></td>
                <td width="10%"><?
		  	if($bd4['isstate']=="1"){
