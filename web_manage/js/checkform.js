...
function changebig(locationid){
	document.dforms.cityid.length = 0;
	document.dforms.cityid.options[0] = new Option('-请选择-','');
	for (i=0; i<subcat.length; i++){
		if (subcat[i][0] == locationid){
			document.dforms.cityid.options[document.dforms.cityid.length] = new Option(subcat[i][1], subcat[i][2]);
		}
	}
}
