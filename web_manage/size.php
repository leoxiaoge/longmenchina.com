...
if ($action=="confirm"){
	...
	$sql="update `{$tablepre}size` SET isstate=NOT(isstate) WHERE id=".$id;
	if($db->sql_query($sql)){
	...
	for($i=0;$i<$del_num;$i++){
		$insertid=$db->delete("size"," id=".$checkid[$i]."");
...
$sql="select * FROM `{$tablepre}size` where 1=1 {$sqlkey} order by disorder desc,id asc";
