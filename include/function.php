...
//根据条件获取内容
if(!function_exists('get_zd_name_title')){
	function get_zd_name_title($zd,$table='users',$where) {
	global $db,$tablepre; 
		echo $where;
		$sql= "SELECT {$zd} FROM `{$tablepre}{$table}` WHERE 1=1 and title='".$where."' ";
		echo $sql;
 		$result=$db->sql_query($sql);
		if($arr=$db->sql_fetchrow($result)){
			return $arr[$zd];
...
//获取左侧导航栏目加高亮选中
function get_nav_links($pid=2,$ty,$url="list.php",$num=100) {
	global $db,$tablepre,$srt; 
	$sql= "SELECT * FROM `{$tablepre}news_cats` WHERE pid={$pid} AND isstate=1 order by disorder Asc,id ASC limit 0,$num";
	 //echo $sql;
  	$result=$db->sql_query($sql);
	$i=0;
  	while($bd=$db->sql_fetchrow($result)){
		...
		if($bd['weblinkurl']) $surl=$bd['weblinkurl'];else $surl="{$url}?pid={$bd['pid']}&ty={$bd['id']}";
 		...
 		$str.="<span class=\"{$s}\">{$bd['catname']}</span>";
   	}
 	return $str;
	unset($sql,$result,$bd);
}
...
//获取左侧导航栏目加高亮选中
function get_nav_links2($pid=2,$ty,$url="list.php",$num=100) {
	global $db,$tablepre,$srt; 
	$sql= "SELECT * FROM `{$tablepre}news_cats` WHERE pid={$ty} AND isstate=1 order by disorder Asc,id ASC limit 0,$num";
	 //echo $sql;
  	$result=$db->sql_query($sql);
	$i=0;
  	while($bd=$db->sql_fetchrow($result)){
		...
		<li class=\"cle\">
			<p class=\"tit\">
				<span>{$bd['catname']}</span>
			</p>
			<div class=\"c\">";

				$sql2= "SELECT * FROM `{$tablepre}news` WHERE pid={$pid} and ty={$ty} and tty={$bd['id']} AND isstate=1 order by disorder Asc,id ASC limit 0,50";
				//echo $sql;
				$result2=$db->sql_query($sql2);
				while($bd2=$db->sql_fetchrow($result2)){
					$url=vrt($bd2['id'],$srt,"view");
					
					$str.="<div class=\"fl li\"><a href=\"{$url}\" target=\"_blank\"><i></i>{$bd2['title']}</a></div>";
				}
...
//服务项目
function get_pic_news_lists_isgood($pid=11,$ty=37,$num=1){
	global $db,$upload_picpath,$tablepre,$srt; 
	if($pid) $sqlkey=" AND pid={$pid}";
	if($ty) $sqlkey.=" AND ty={$ty}";
 
	$sql= "SELECT * FROM `{$tablepre}news` WHERE isstate=1 and isgood=1 ".$sqlkey." order by disorder desc,id asc limit 0,$num";
 	$result=$db->sql_query($sql);
 	while($bd=$db->sql_fetchrow($result)){
		if(!empty($bd['img1'])) $Img=$bd['img1']; else $Img="nopic.jpg";
		$url=vrt($bd['id'],$srt,"view");
		...
		<div class=\"fr dect\">
			<p class=\"t\">{$bd['title']}</p>
			<div class=\"info cle\">
				<span class=\"time fl\">{$sendtime}</span>
				<span class=\"lll fl\">{$bd['hits']}次浏览</span>
			</div>
			<div class=\"c\">
				{$nr}
				<a href=\"{$url}\">查看更多</a>
...
function get_ad_img_list3($pid=6,$ty=29,$num=10){
	...
	$sql= "SELECT * FROM `{$tablepre}news` WHERE isstate=1 ".$sqlkey." order by id asc,id asc limit 0,$num";
 	$result=$db->sql_query($sql);
 	...
 	while($bd=$db->sql_fetchrow($result)){
		...
		if(!empty($bd['img1'])) $Img=$bd['img1']; else $Img="nopic.jpg";
 		...
 		$str.="
		<div class=\"item\">
			<p class=\"aa1\">{$bd['ftitle']}</p>
			<p class=\"aa2\">{$bd['seodescription']}</p>
